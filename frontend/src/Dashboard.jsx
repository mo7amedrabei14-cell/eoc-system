                    ...
                    <div key={ci.id} className="bg-[var(--surface-4)] border border-[var(--border)] p-4 rounded-xl">
                      ...
                      {ci.routes.map((cr, rIndex) => (
                        <div key={cr.id} className="flex flex-col md:flex-row w-full border border-[var(--border)] rounded-lg overflow-hidden mb-2 bg-[var(--surface-3)]">
                          ...
                          <div className="w-full md:w-auto flex border-l border-[var(--border)]"><input id={`r_dep_cust_${ciIndex}_${rIndex}`} type="time" dir="ltr" defaultValue={cr.departure_time || ''} className="bg-transparent text-white px-2 w-28 text-center" /></div>
                          <div className="w-full md:w-auto flex"><input id={`r_arr_cust_${ciIndex}_${rIndex}`} type="time" dir="ltr" defaultValue={cr.arrival_time || ''} className="bg-transparent text-white px-2 w-28 text-center" />{ci.routes.length > 1 && <button onClick={() => removeRouteFromCustom(ci.id, cr.id)} className="px-3 text-[var(--faint)]"><TrashIcon /></button>}</div>
                        </div>
                      ))}
                    </div>
                  ))}
